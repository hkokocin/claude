---
name: build
description: Write tests (RED), implement (GREEN), and refactor. Takes the specification from /refinement and the test plan from /test_plan and runs 3 sequential subagents.
...
The specification from `/refinement` and the test plan from `/test_plan` fully describe the target behaviour.

**CRITICAL: Stop on failure. Do NOT proceed to the next step if the current step fails.**

## Step 0: Gather Context

1. Read the conversation history to extract the approved **specification** from `/refinement` and the approved **test plan** from `/test_plan`.
2. If no specification or test plan is found, stop and tell the user to run the missing phase (`/refinement`, then `/test_plan`) first.
